...
# Makefile.am doesn't exclude this test on non-Linux. Use backquotes in this
# check to keep it compatible with /bin/sh on Solaris 10.
test "`uname -s`" = "Linux" || exit 77
...
# A transient systemd unit gives the test its own cgroup without changing
# the host's cgroup hierarchy. --pipe is needed to capture xz's output.
#
...
# Check that systemd-run is usable. This way failures from xz won't be
# confused with failures from systemd-run.
$SYSTEMD_RUN true || exit 77
...
# memory.
output=$($SYSTEMD_RUN "$XZ" -M0 --robot --info-memory) || exit 1
...
output=$($SYSTEMD_RUN -p CPUQuota=277% \
	"$XZ" -M0 --memlimit-compress=50% --robot --info-memory) || exit 1
